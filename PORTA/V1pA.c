...
void main(void){
    ...
    uchar serNum[5];
    uchar Date[16];  //Dados a serem gravados no cart„o
    ...
    while(TRUE){
        ...
        if( Read_MFRC522( CommIrqReg ) & 0x20){
            output_high(LEDW); //Detector de cart„o
            delay_ms(100);
        }
        ...
        if(ok < 2 && input(LEDB) == 0){
            ...
            if(ok == 1){
                ...
                Date[15] = Date[15] - 1;  //decrementa o valor em write data na posiÁ„o 15 (ultima posiÁ„o)
